...
class FDBayes(Posterior):
    ...
    def compute_gibbs_probabilities(self, param, X_configs):
        """
        Compute Gibbs measure probabilities for given configurations.
        
        Parameters:
        -----------
        param : torch.Tensor
            Model parameters
        X_configs : torch.Tensor
            Configurations to evaluate (n_configs, n_features)
            
        Returns:
        --------
        probs : torch.Tensor
            Gibbs probabilities for each configuration
        """
        # Compute statistics for the configurations
        SX_m_configs = self.stat_m(X_configs)
        SX_p_configs = self.stat_p(X_configs)
        
        # Compute ratios
        ratio_m_configs = self.ratio_m(param, SX_m_configs)
        ratio_p_configs = self.ratio_p(param, SX_p_configs)
        
        # Gibbs measure is proportional to the ratio_p
        # We normalize to get proper probabilities
        log_probs = torch.log(ratio_p_configs + 1e-8)  # Add small epsilon for numerical stability
